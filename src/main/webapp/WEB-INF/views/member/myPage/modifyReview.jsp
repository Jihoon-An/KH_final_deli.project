...
<%--    리뷰사진--%>
    <div class="imgs">
        <c:choose>
            <c:when test="${review_dto}">
                <c:forEach var="i" items="${review_dto}">
                    <label class="review_img_label">
                        <img src="/resources/img/review/${i.rev_sysname}">
...
<%--    <input type="hidden" value="${review_dto.rev_content}">--%>
    ...
    <div>
      ...
      <c:choose>
        <c:when test="${not empty orders_dto.menu_list}">
          <c:forEach var="i" items="${orders_dto.menu_list}">
